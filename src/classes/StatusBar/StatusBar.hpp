...
namespace GOTHIC_NAMESPACE
{
  ...
  int StatusBar::GetValueFromItem(oCItem *item, int atr)
  {
    ...
    for (int i = 0; i < symbols.GetNumInList(); i++)
    {
      ...
      if (Union::StringANSI(symbols[i]).EndsWith("_FULL"))
      {
        return player->GetAttribute(atr);
      }
      else if (Union::StringANSI(symbols[i]).EndsWith("_PERC"))
      {
  ...
  void StatusBar::PrintValue(oCNpc *npc)
  {
    ...
    zSTRING str;
    if (bar == ogame->swimBar)
      str = Union::StringANSI::Format("{0}/{1}", (int)(bar->currentValue / 100), (bar->maxHigh / 100));
    else
      str = Union::StringANSI::Format("{0}/{1}", (int)bar->currentValue, (int)bar->maxHigh);
  ...
  {
    if (userPos.GetCount() != 4)
      return;

    if (!IsBarActive())
      return;

    int x1 = userPos[0].ConvertToInt();
    int y1 = userPos[1].ConvertToInt();

    if (x1 < 0 || x1 > 8192 || y1 < 0 || y1 > 8192)
      return;

    int x2 = userPos[2].ConvertToInt() - x1;
    int y2 = userPos[3].ConvertToInt() - y1;
