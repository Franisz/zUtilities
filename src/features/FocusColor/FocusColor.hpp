...
namespace GOTHIC_NAMESPACE
{
  ...
  void __fastcall zCView_Print(zCView *_this, void *vtable, int x, int y, zSTRING const &text)
  {
    ...
    Hook_zCView_Print(_this, vtable, x, y, text);
  }
  ...
  int FocusColor::GetAbsolutionLevel(oCNpc *slf)
  {
    ...
    if ((NPCTYPE_OCAMBIENT && slf->npcType == NPCTYPE_OCAMBIENT->single_intdata) || (NPCTYPE_OCMAIN && slf->npcType == NPCTYPE_OCMAIN->single_intdata))
    {
      if (slf->guild == NPC_GIL_PALADIN || slf->guild == NPC_GIL_MILIZ || slf->guild == NPC_GIL_VOLK)
        if (ABSOLUTIONLEVEL_OldCamp)
          return ABSOLUTIONLEVEL_OldCamp->single_intdata;
    }
    else if (slf->guild == NPC_GIL_PALADIN || slf->guild == NPC_GIL_MILIZ || slf->guild == NPC_GIL_VOLK)
    {
      if (ABSOLUTIONLEVEL_City)
        return ABSOLUTIONLEVEL_City->single_intdata;
    }
    else if (slf->guild == NPC_GIL_FEUERKREIS || slf->guild == NPC_GIL_NOVIZE)
    {
      if (ABSOLUTIONLEVEL_Monastery)
        return ABSOLUTIONLEVEL_Monastery->single_intdata;
    }
    else if (slf->guild == NPC_GIL_BAUERN)
    {
      if (ABSOLUTIONLEVEL_Farm)
        return ABSOLUTIONLEVEL_Farm->single_intdata;
    }
    else if ((NPCTYPE_BL_AMBIENT && slf->npcType == NPCTYPE_BL_AMBIENT->single_intdata) || (NPCTYPE_BL_MAIN && slf->npcType == NPCTYPE_BL_MAIN->single_intdata))
    {
      if (ABSOLUTIONLEVEL_BL)
        return ABSOLUTIONLEVEL_BL->single_intdata;
    }
  ...
  {
    if (slf->GetAivar("AIV_NpcSawPlayerCommit") == CRIME_MURDER)
      return true;
    ...
    if (AttackReason == 19)
      return true;
    if (slf->GetAivar("AIV_DropDeadAndKill"))
      return true;
  ...
  zCOLOR FocusColor::NpcColor(oCNpc *npc)
  {
...
#if ENGINE >= Engine_G2
    if (npc->IsHostile(player) && npc->GetPermAttitude(player) == NPC_ATT_HOSTILE || (npc->enemy == player && npc->IsAIState(ZS_Attack) && HasReasonToKill(npc)))
    ...
    if (npc->GetAivar("AIV_NpcSawPlayerCommit") && !(npc->GetAivar("AIV_NpcSawPlayerCommit") < CRIME_MURDER && npc->GetAivar("AIV_NpcSawPlayerCommitDay") < day - 2) && !(npc->GetAivar("AIV_CrimeAbsolutionLevel") < GetAbsolutionLevel(npc)))
      return zCOLOR(255, 180, 0);
#else
    if ((npc->IsHostile(player) && npc->GetPermAttitude(player) == NPC_ATT_HOSTILE) || (npc->enemy == player && npc->IsAIState(ZS_Attack) && npc->GetAivar("AIV_ATTACKREASON")))
      return zCOLOR(255, 0, 0);
  ...
  void FocusColor::TryShowProt(oCNpc *npc)
  {
    ...
    if (!isImmune)
    {
      ...
      protView->Print(startX + margin * iconNr + size * (iconNr++ - 1), fontY, zSTRING(protection));
    }
  }
